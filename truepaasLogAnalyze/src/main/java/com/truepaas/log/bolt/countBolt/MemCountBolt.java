 ...
 */
public class MemCountBolt extends BaseBasicBolt {
    private Date baseDate;
    private Date firstBaseDate;
    ...
    public void execute(Tuple input, BasicOutputCollector collector) {
        ...
        double mem= Double.valueOf(log.getMemory_bytes())/1024/1024;
        if(map.containsKey(appId)){
            Double dou=(Double) map.get(appId).getIndicator_value();
            map.get(appId).setIndicator_value((dou+mem)/2);
        }else{
            //定义countLog
            ...
            cLog.setIndicator("mem count");
            cLog.setIndicator_value(mem);
