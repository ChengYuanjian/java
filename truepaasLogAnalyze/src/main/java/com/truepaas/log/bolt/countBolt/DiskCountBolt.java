 ...
 */
public class DiskCountBolt extends BaseBasicBolt {
    ...
    public void execute(Tuple input, BasicOutputCollector collector) {
        ...
        double disk= Double.valueOf(log.getDisk_bytes())/1024/1024;
        if(map.containsKey(appId)){
            Double dou=(Double)map.get(appId).getIndicator_value();
            map.get(appId).setIndicator_value((dou+disk)/2);
        }else{
            //定义countLog
            ...
            cLog.setIndicator("disk count");
            cLog.setIndicator_value(disk);
